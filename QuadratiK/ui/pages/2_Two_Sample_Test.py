...
def run_twosample_test(h_val,num_iter,b,X,Y):
    return kt(h=h_val, num_iter=num_iter, b=b, 
              centering_type="nonparam").test(x=X, y=Y)
...
x_data = st.file_uploader("Choose data file for X", accept_multiple_files=False, type=[
                          '.txt', '.csv'],)
...
y_data = st.file_uploader("Choose data file for Y", accept_multiple_files=False, type=[
                          '.txt', '.csv'])
...
if (y_data) and (x_data) is not None:
    ...
    with st.spinner('getting results ready...'):
        ...
        try:
            ...
            res = pd.DataFrame()
            res['Value'] = [two_sample_test.test_type_, two_sample_test.test_statistic_,
                            two_sample_test.cv_, two_sample_test.h0_rejected_]
            ...
            st.download_button("Click to Download the test results", csv_res,
                               "Two_Sample_Test_results.csv", "text/csv", key='download-txt')
