...
def run_normality_test(h_val, num_iter, b, x):
    return kt(h=h_val, num_iter=num_iter, b=b,
              centering_type="nonparam").test(x=x)
...
if data is not None:
    ...
    num_iter = st.number_input(
        'Enter a value of number of iterations to be used for critical value estimation', value=150)
    ...
    with st.spinner('getting results ready...'):
        ...
        try:
            ...
            res = pd.DataFrame()
            res['Value'] = [norm_test.test_type_, norm_test.test_statistic_,
                            norm_test.cv_, norm_test.h0_rejected_]
